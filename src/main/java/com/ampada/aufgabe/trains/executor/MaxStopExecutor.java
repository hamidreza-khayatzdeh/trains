 ...
 */
@Component
public class MaxStopExecutor extends AbstractExecutor implements TrainExecutor {
    ...
    public ResponseDto execute(final String requestContent) throws NoSuchRouteException {
        ...
        final String routeLine = requestContent.substring(11);
        final String[] nodes = routeLine.split("-");
        try {
            int stops = Integer.valueOf(nodes[2]);
            int result = trainService.calculateNumberOfPathsWithMaxStops(nodes[0], nodes[1], stops);
