 ...
 */
@Component
public class ExactStopExecutor extends AbstractExecutor implements TrainExecutor {
    ...
    public ResponseDto execute(final String requestContent) throws NoSuchRouteException {
        ...
        final String routeLine = requestContent.substring(13);
        final String[] nodes = routeLine.split("-");
        try {
            int stops = Integer.valueOf(nodes[2]);
            int result = trainService.calculateNumberOfPathsWithExactStops(nodes[0], nodes[1], stops);
